...
# Initialize DagsHub (keeping your original setup)
dagshub.init(repo_owner='maverick011', repo_name='Uber-Demand-Prediction', mlflow=True)
mlflow.set_tracking_uri("https://dagshub.com/maverick011/Uber-Demand-Prediction.mlflow")
...
# Load model directly from local joblib file (avoiding MLflow model registry issues)
current_path = Path(__file__)
root_path = current_path.parent.parent
model_path = root_path / "models/model.joblib"
model = joblib.load(model_path)

# Keep the original logic but comment out the problematic MLflow call
# model_uri = load_model_information("run_information.json")["model_uri"]
# model = mlflow.sklearn.load_model(model_uri)  # This causes DagsHub issues
